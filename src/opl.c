...
static void updateMenuFromGameList(opl_io_module_t *mdl)
{
    ...
    if (count > 0) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            char path[256];
            snprintf(path, sizeof(path), "%sCFG%s%s.cfg", ethGetPrefix(), "\\", mdl->support->itemGetStartup(i));
            config_set_t *config = configAlloc(0, NULL, path);
            configRead(config);

            //try to get title from game config file, else display name from file name
            const char *title;
            if (configGetStr(config, CONFIG_ITEM_TITLE, &title) != 0)
              gup->submenu.text = title;
            else
